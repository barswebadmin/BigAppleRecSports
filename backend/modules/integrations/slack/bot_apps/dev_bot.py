"""Slack Bolt app initialization for Dev bot."""
...
class DevBot(App):
    ...
    - dev_bot.send_message() instead of dev_bot.client.send_message()
    """
    ...
    def lookup_user(
        ...
        Examples:
            # By email
            user = dev_bot.lookup_user("user@example.com")
            
            # By user ID
            user = dev_bot.lookup_user("U1234567890")
            
            # With SlackUserIdentifier
            user = dev_bot.lookup_user(SlackUserIdentifier(email="user@example.com"))
        """
    ...
    def lookup_channel(
        ...
        Examples:
            # By name
            channel = dev_bot.lookup_channel("general")
            channel = dev_bot.lookup_channel("#kickball-leadership")
            
            # By ID
            channel = dev_bot.lookup_channel("C092RU7R6PL")
        """
...
dev_bot = DevBot(
    client=SlackClient(
        token=SlackConfig.Bots.Dev.token
    ),
    signing_secret=SlackConfig.Bots.Dev.signing_secret,
