...
def handle_update_bars_leadership_command(ack: Ack, command: dict):
...
def handle_get_user_ids_command(ack: Ack, command: dict):
...
def handle_csv_user_lookup_submission(ack: Ack, respond: Respond, context: BoltContext, view: dict):
    ...
    try:
        ...
        if not emails:
            _post_error_message(
                respond,
                f"No valid emails found in column {chr(65 + column_index)}"
            )
            return
        
        results = lookup_user_ids_by_emails(leadership_bot.client, emails)
        
        _post_results_message(respond, results)
...
def _post_results_message(
    ...
    Args:
        respond: Bolt Respond context for ephemeral messages
        results: Dictionary mapping email -> slack_user_id (or None if not found)
    """
...
def _post_error_message(
    ...
    """
    builder = GenericMessageBuilder()
    respond(blocks=[builder.section(f"❌ *Error*\n{error_message}")])


@leadership_bot.event("file_shared")
def handle_file_shared(event: dict, logger):
    ...
    try:
        file_info_response = leadership_bot.files_info(file=file_id)
        file_info = file_info_response.get("file", {}) if isinstance(file_info_response, dict) else {}
        ...
        csv_data = download_and_parse_csv(file_url, leadership_bot)
        ...
        _post_file_upload_prompt(
            user_id, channel_id, file_id, file_name,
...
def _post_file_upload_prompt(
    user_id: str, channel_id: str, file_id: str,
    ...
    response = leadership_bot.chat_postEphemeral(
        channel=channel_id,
        user=user_id,
        blocks=blocks,
...
def handle_confirm_file_process(ack: Ack, respond: Respond, context: BoltContext, action: dict):
    ...
    try:
        ...
        if not file_url:
            _post_error_message(respond, "Could not get file URL")
            return
        
        csv_data = download_and_parse_csv(file_url, leadership_bot)
        
        if not csv_data:
            _post_error_message(respond, "Could not download CSV file")
            return
        
        blocks = _process_leadership_csv(leadership_bot, csv_data, header_row, position_col, email_col)
        ...
        respond(
            text="Error",
            blocks=[builder.section(f"❌ *Error* - {str(e)}")],
            replace_original=True
        )


@leadership_bot.action("cancel_file_process")
def handle_cancel_file_process(ack: Ack, respond: Respond):
    ...
    respond(
        text="Cancelled",
        blocks=[builder.section("❌ Processing cancelled")],
        replace_original=True
    )


@leadership_bot.action("edit_file_columns")
def handle_edit_file_columns(ack: Ack, context: BoltContext, action: dict, body: dict):
    ...
    leadership_bot.views_open(trigger_id=body["trigger_id"], view=modal_view)


@leadership_bot.view("column_selection_modal")
def handle_column_selection_modal(ack: Ack, respond: Respond, context: BoltContext, view: dict):
    ...
    try:
        ...
        blocks = _process_leadership_csv(leadership_bot, csv_data, header_row_index, position_col, email_col)
...
def _process_leadership_csv(
    ...
    results = enrich_hierarchy(client, hierarchy)
