"""
Legacy Slack message building utilities for refund workflows.
Extracted from message_builder.py to separate refund-specific logic.
"""
...
class SlackMessageBuilderLegacy:
    """Legacy refund message building logic."""
    
    def __init__(self, parent_builder):
        """
        Initialize with reference to parent builder for accessing shared methods.
        
        Args:
            parent_builder: SlackMessageBuilder instance for accessing helper methods
        """
    ...
    def create_refund_decision_message(
        ...
        try:
            ...
            if original_data.get("product_display"):
                ...
                product_url = self.parent.build_hyperlink(build_product_url(product_id), product_title) if product_id else "#"
            ...
            if original_data.get("order_number_display"):
                ...
                order_url = self.parent.build_hyperlink(build_order_url(order_id), order_number)
            ...
            message_text = ""
            message_text += (
                f"*Request Type*: {self.parent._get_request_type_text(refund_type)}\n\n"
            )
            customer_data = None
            if order and "customer" in order:
                customer_data = order["customer"]
            message_text += self.parent._get_requestor_line(
            ...
            message_text += self.parent._get_sheet_link_line(sheet_link)
    ...
    def build_success_message(
        ...
        try:
            ...
            product_url = self.parent.build_hyperlink(build_product_url(product_id), product_title) if product_id else "#"
            sport_mention = self.parent.get_group_mention(product_title)
            ...
            if refund_text:
                ...
                message_text += "\n"
            message_text += self.parent._get_optional_request_notes(request_notes)
    ...
    def build_email_mismatch_message(
        ...
        try:
            ...
            order_url = self.parent.build_hyperlink(
            ...
            if requestor_full_name and customer_data and customer_data.get("id"):
                customer_url = self.parent.build_hyperlink(build_customer_url(customer_data["id"]), requestor_full_name)
            ...
            if order_data and "order" in order_data:
                ...
                if order_id:
                    ...
                    order_url = self.parent.build_hyperlink(build_order_url(order_id), formatted_order_number)
                    message_text += f"*Order Number:* {order_url}\n\n"
                else:
