...
class DirectoryServiceMixin:
    """Mixin class containing Google Directory API methods."""
    ...
    def list_all_users(
        ...
        users_dicts = self._paginate_api_call(
            self.directory_service.users().list,
            result_key='users',
            **params
        )
    ...
    def get_user(
        ...
        """
        user_dict = self.directory_service.users().get(userKey=user_email).execute()  # type: ignore[attr-defined]
    ...
    def list_all_groups(
        ...
        groups_dicts = self._paginate_api_call(
            self.directory_service.groups().list,
            result_key='groups',
            **params
        )
    ...
    def get_group(
        ...
        group_dict = self.directory_service.groups().get(groupKey=group_email).execute()  # type: ignore[attr-defined]
    ...
    def list_group_members(
        ...
        members_dicts = self._paginate_api_call(
            self.directory_service.members().list,
            result_key='members',
            **params
        )
    ...
    def add_member_to_group(
        ...
        result_dict = self.directory_service.members().insert(
    ...
    def is_member_of_group(
        ...
        """
        try:
            result_dict = self.directory_service.members().get(
                groupKey=group_email,
                memberKey=user_email
            ).execute()
