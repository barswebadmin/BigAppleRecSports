...
This service now uses the new GoogleApiClient for generic API execution.
"""
...
from ..models.requests import GetGroupsRequest
from ._google_api_service_builder import build_google_api_service
from ..base_methods import handle_http_errors, paginate_api_call
from ..models.google_directory_resources import (
    GroupResource, MemberResource, UserResource, GroupWithMembers
)
...
class GoogleDirectoryService():
    ...
    This service uses the new GoogleApiClient for generic API execution with
    centralized credential management and error handling. It provides methods
    for interacting with Google Workspace Directory API.
    """

    def __init__(self, client: Optional["GoogleApiClient"] = None):
        # Use provided client or None (will be set later if needed)
        self.client = client

        # Base scopes for Directory API
        self.base_scopes = DirectoryScopes()

        # Legacy service for backward compatibility with existing methods
        required_scopes = [
    ...
    def get_groups(self, request: GetGroupsRequest) -> List[GroupResource]:
        """
        Get Google Workspace groups using the new client architecture.

        Args:
            request: GetGroupsRequest model with filtering parameters

        Returns:
            List of GroupResource models

        Raises:
            Exception: For Google API errors (handled centrally by client)
        """
        if not self.client:
            raise ValueError("Client not provided. Use GoogleDirectoryService(client=your_client)")
        ...
        if request.customer:
            ...
            params['customer'] = 'my_customer'  # Default
        ...
        if request.user_key:
            params['userKey'] = request.user_key

        # Use the new client to execute the request
        response = self.client.execute_request(
            service_name="admin",
            version="directory_v1",
            scopes=[self.base_scopes.groups_readonly],
            resource="groups",
            method="list",
            params=params
        )

        # Parse response and convert to models
        groups_data = response.get('groups', [])
        groups = [GroupResource(**group_data) for group_data in groups_data]

        logger.info("Retrieved %d groups using new client architecture", len(groups))
        return groups
    ...
    def get_user(
        ...
        logger.info("Found user: %s", user.primary_email)
    ...
    def create_user(
        ...
        logger.info("Created user: %s", user.primary_email)
    ...
    def get_group(
        ...
        Example:
            >>> service = GoogleDirectoryService()
            >>> result = service.get_group("team@example.com")
            >>> group = result.group
        ...
        if include_members:
            try:
                ...
                logger.warning("Could not list members for %s: %s", group_email, e)
                members = []
    ...
    def list_group_members(
        ...
        Example:
            >>> service = GoogleDirectoryService()
            >>> members = service.list_group_members("team@example.com")
            >>> print(f"Group has {len(members)} members")
            >>> owners = service.list_group_members("team@example.com", roles=["OWNER"])
        """
        ...
        logger.info("Found %d members in group %s", len(members), group_email)
    ...
    def add_member_to_group(
        ...
        Example:
            >>> service = GoogleDirectoryService()
            >>> result = service.add_member_to_group("team@example.com", "user@example.com")
        ...
        try:
            ...
            logger.info("Added %s to group %s with role %s", user_email, group_email, role)
    ...
    def remove_member_from_group(
        ...
        Example:
            >>> service = GoogleDirectoryService()
            >>> service.remove_member_from_group("team@example.com", "user@example.com")
        """
        self.members().delete(
            groupKey=group_email,
    ...
    def create_group(
        ...
        logger.info("Created group: %s", group.email)
    ...
    def is_member_of_group(
        ...
        Example:
            >>> service = GoogleDirectoryService()
            >>> if service.is_member_of_group("team@example.com", "user@example.com"):
