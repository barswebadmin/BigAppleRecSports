...
from backend.modules.integrations.google.models.google_mail_resources import EmailMessage, EmailSearchResult
...
class GoogleMailService():
    def __init__(self):
        ...
        print("[DEBUG] GoogleMailService.__init__: Building Gmail service", file=sys.stderr)
        logger.debug("GoogleMailService.__init__: Building Gmail service")
        self.service = build_google_api_service('gmail', 'v1', required_scopes)
        self.required_scopes = required_scopes  # Store for error diagnostics
        print(f"[DEBUG] GoogleMailService.__init__: Service built, type: {type(self.service)}", file=sys.stderr)
        logger.debug(f"GoogleMailService.__init__: Service built, type: {type(self.service)}")
        
        print("[DEBUG] GoogleMailService.__init__: Accessing service.users()", file=sys.stderr)
        logger.debug("GoogleMailService.__init__: Accessing service.users()")
        self.users = self.service.users()  # type: ignore[attr-defined]
        print(f"[DEBUG] GoogleMailService.__init__: users() accessed, type: {type(self.users)}", file=sys.stderr)
        logger.debug(f"GoogleMailService.__init__: users() accessed, type: {type(self.users)}")
        
        # Gmail API structure: service.users().messages(), not service.messages()
        # Methods use self.users().messages() directly, so no need to cache messages here
    ...
    def find_emails(
        ...
        while total_fetched < max_results:
            ...
            response = self.users().messages().list(**request_params).execute()
    ...
    def _get_message_details(self, message_id: str, include_body: bool = True) -> Optional[EmailMessage]:
        ...
        message = self.users().messages().get(
            userId='me',
            id=message_id,
            format=format_type
        ).execute()
