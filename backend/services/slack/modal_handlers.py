...
class SlackModalHandlers:
    ...
    def __init__(self, api_client: Union[SlackApiClient, Any], gas_webhook_url: str):
        self.api_client = api_client
        self.gas_webhook_url = gas_webhook_url
    
    async def show_deny_refund_request_modal(self, request_data: Dict[str, str], channel_id: str, 
    ...
    def _build_deny_request_modal_blocks(self, raw_order_number: str, requestor_email: str, 
                                       first_name: str, last_name: str, refund_type: str) -> List[Dict[str, Any]]:
    ...
    def _show_modal_to_user(self, trigger_id: str, modal_title: str, modal_blocks: List[Dict[str, Any]], 
    ...
    async def _send_denial_email_via_gas(self, order_number: str, requestor_email: str, 
                                        first_name: str, last_name: str, custom_message: str,
                                        include_staff_info: bool, slack_user_name: str, slack_user_id: str):
        """Send denial email request to Google Apps Script"""
        import requests
        ...
        try:
            ...
            try:
                response = requests.post(self.gas_webhook_url, json=payload, timeout=30, verify=True)
                response.raise_for_status()
                print(f"✅ Denial email request sent to GAS successfully (with SSL)")
                ...
                response = requests.post(self.gas_webhook_url, json=payload, timeout=30, verify=False)
                response.raise_for_status()
                print(f"✅ Denial email request sent to GAS successfully (without SSL)")
                print(f"📥 GAS Response: {response.status_code} - {response.text}")
                
        except Exception as e:
            logger.error(f"❌ Failed to send denial email via GAS: {str(e)}")
            print(f"❌ Full error details: {e}")
            print(f"🔗 Tried URL: {self.gas_webhook_url}")
            print(f"📦 Payload was: {json.dumps(payload, indent=2)}")
    
    def _build_denial_confirmation_message(self, order_number: str, requestor_email: str, 
                                         first_name: str, last_name: str, slack_user_name: str,
                                         custom_message_provided: bool, include_staff_info: bool) -> str:
        """Build the Slack message confirming the denial was processed"""
        current_time = format_date_and_time(datetime.now(timezone.utc))
        
        message = f"🚫 *Refund Request Denied*\n\n"
        ...
        if custom_message_provided:
            message += f"📧 *Custom denial message sent to requestor.*\n"
        else:
            message += f"📧 *Default denial message sent to requestor.*\n"
        
        if include_staff_info:
            message += f"👤 *Staff contact info included for direct follow-up.*"
        else:
            message += f"📬 *Replies will go to web@bigapplerecsports.com*"
        
        return message
    
    async def _update_slack_message(self, channel_id: str, message_ts: str, message_text: str) -> Dict[str, Any]:
