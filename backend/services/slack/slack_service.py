...
try:
    ...
    from .builders import SlackMessageBuilder, ModernMessageBuilder, SlackCacheManager, SlackMetadataBuilder, SlackMentionResolver, SlackMessageParsers, SlackOrderHandlers
...
class SlackService:
    ...
    def __init__(self, token: Optional[str] = None, default_channel: Optional[str] = None):
        ...
        self.mention_resolver = self._get_mention_resolver()
    ...
    def _resolve_bot_token(self, bot_name: str) -> Optional[str]:
        """Resolve bot name to token using config."""
        return self.config.Token.resolve_bot_token(bot_name)
    
    def _resolve_channel_id(self, channel: str) -> Optional[str]:
        """Resolve channel name to ID using config."""
        return self.config.Channel.resolve_channel_id(channel)
    ...
    def _get_mention_resolver(self):
        """Get mention resolver instance."""
        return SlackMentionResolver(self._get_sport_groups())
    ...
    def send_message(
        ...
        bot: str = 'registrations',  # Bot name (replaces 'as' parameter)
        **kwargs  # For backwards compatibility
        ...
        Args:
            channel: Channel name ('joe-test', 'registration-refunds') or ID
            message_text: Plain text message (used as fallback if blocks provided)
            blocks: Slack blocks for rich formatting
            bot: Bot name ('registrations', 'refunds', etc.)
        ...
        """
        # Get bot name from parameter, with fallback to 'as' in kwargs for backwards compatibility
        bot_name = bot or kwargs.get('as', 'registrations')
        bot_name = bot_name.lower().strip()
        
        # Resolve bot name to token
        token = self._resolve_bot_token(bot_name)
        if not token:
        ...
        if blocks:
            return slack_client.send_message_with_blocks(
                channel_id, blocks, message_text or "Message", slack_text, metadata, thread_ts
            )
        else:
            return slack_client.send_message(
                channel_id, message_text or "Empty message", action_buttons, slack_text, metadata, thread_ts
            )
