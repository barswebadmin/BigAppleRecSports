...
class ShopifyHandler:
    ...
    def handle_product_update_webhook(self, body: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            shopify_store_url = f"https://{self.product_parser.shopify_store}/products/{product_handle}" if product_handle else ""
            ...
            logger.info(f"🚨 Sold Out Status: {'✅ SOLD OUT' if has_zero_inventory else '❌ Still has inventory'}")
            
            if not has_zero_inventory:
                logger.info(f"⏭️ Product still has {total_inventory} units in stock - no waitlist action needed")
                return {
                    "success": True, 
            ...
            logger.info(f"🎯 Product is sold out - processing for waitlist form...")
            ...
            if gas_result.get("success"):
                ...
                logger.error(f"❌ Failed to add '{product_title}' to waitlist form: {gas_result}")
