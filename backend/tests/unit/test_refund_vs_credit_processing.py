...
*Estimated Refund Due:* $19.00"""
    ...
    @patch(
        "services.slack.slack_refunds_utils.SlackRefundsUtils.build_comprehensive_success_message"
    )
    @patch(
    ...
    async def test_refund_processing_flow(self, mock_update_slack, mock_build_message):
        ...
        assert not call_args["order_cancelled"]  # Based on request data
    ...
    def test_refund_calculation_vs_credit_calculation(self):
        ...
        self.mock_orders_service.calculate_refund_due.side_effect = (
            self._mock_calculate_based_on_type
        )
    ...
    def test_refund_calculation_with_different_timeframes(self):
        ...
        late_timestamp = datetime(
            2024, 10, 10, tzinfo=timezone.utc
        )  # Close to season start

        self.mock_orders_service.calculate_refund_due.side_effect = (
            self._mock_calculate_timeframe_based
        )
    ...
    def _mock_calculate_timeframe_based(
        self, order_data, refund_type, request_timestamp
    ):
    ...
    def test_calculation_logging_differs_by_type(self):
        ...
        self.mock_orders_service.calculate_refund_due.side_effect = (
            self._mock_calculate_with_logging
        )
