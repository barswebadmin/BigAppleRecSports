...
class TestRefundsEndpointJSONValidation:
    ...
    def test_pydantic_model_validation_valid_refund(self):
        ...
        assert (
            request.sheet_link
            and "11oXF8a7lZV0349QFVYyxPw8tEokoLJqZDrGDpzPjGtw" in request.sheet_link
        )
...
class TestRefundsEndpointIntegration:
    ...
    def test_refunds_endpoint_success_flow(
        self, mock_orders_service, mock_slack_service
    ):
    ...
    @patch("routers.refunds.slack_service")
    @patch("routers.refunds.orders_service")
    def test_refunds_endpoint_order_not_found(
        self, mock_orders_service, mock_slack_service
    ):
    ...
    @patch("routers.refunds.slack_service")
    @patch("routers.refunds.orders_service")
    def test_refunds_endpoint_email_mismatch(
        self, mock_orders_service, mock_slack_service
    ):
...
class TestRefundsEndpointRealWorldScenarios:
    ...
    def test_frontend_json_with_longer_names(self):
        ...
        frontend_json = {
            "order_number": "12345",
            "requestor_name": {"first": "Jonathan", "last": "Randazzo"},
