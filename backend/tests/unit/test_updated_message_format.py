...
*Attn*: @here"""
    ...
    def test_message_preserves_original_data(self):
        ...
        assert (
            "*Season Start Date*:" in message_text
        )  # May be "Unknown" if not parsed correctly
        # Total Paid may not be preserved if not found in current message regex

        # Check that order and product links are preserved
        assert (
            "https://admin.shopify.com/store/09fe59-3/orders/5876418969694"
            in message_text
        )
    ...
    def test_amount_formatting_with_decimals(self):
        ...
        for amount, expected_format in test_cases:
            ...
            assert (
                f"{expected_format} *refund* issued by <@U0278M72535>" in message_text
            )
    ...
    def test_google_sheets_link_preservation(self):
        ...
        assert (
            "https://docs.google.com/spreadsheets/d/11oXF8a7lZV0349QFVYyxPw8tEokoLJqZDrGDpzPjGtw/edit"
            in message_text
        )
