...
• Open Registration: 12 spots available"""
    ...
    @patch('services.slack.slack_refunds_utils.SlackRefundsUtils.build_completion_message_after_restocking')
    @patch('services.slack.slack_refunds_utils.SlackRefundsUtils.update_slack_on_shopify_success')
    @pytest.mark.asyncio
    async def test_successful_variant_restock(self, mock_update_slack, mock_build_completion, mock_extract_sheet):
        """Test successful variant restock handling"""
        
        # Mock dependencies
        mock_extract_sheet.return_value = "https://docs.google.com/spreadsheets/test"
        mock_build_completion.return_value = "✅ *Refund Processing Complete* - Open Registration restocked"
    ...
    @patch('services.slack.slack_refunds_utils.SlackRefundsUtils.update_slack_on_shopify_success')
    @pytest.mark.asyncio
    async def test_do_not_restock_handling(self, mock_update_slack, mock_build_completion, mock_extract_sheet):
        """Test 'Do Not Restock' button handling"""
        
        # Mock dependencies
        mock_extract_sheet.return_value = "https://docs.google.com/spreadsheets/test"
        mock_build_completion.return_value = "✅ *Refund Processing Complete* - No inventory restocked"
    ...
    def test_action_id_to_variant_name_mapping(self):
        ...
        for action_id, expected_variant_name in test_cases:
            ...
            assert expected_variant_name.lower() in action_id.replace("restock_", "").replace("_", " ").lower()
    ...
    async def test_request_data_validation(self):
        ...
        assert valid_request_data["variantId"].startswith("gid://shopify/ProductVariant/")

    @patch('services.slack.slack_refunds_utils.SlackRefundsUtils.extract_sheet_link')
    @patch('services.slack.slack_refunds_utils.SlackRefundsUtils.build_completion_message_after_restocking')
    @patch('services.slack.slack_refunds_utils.SlackRefundsUtils.update_slack_on_shopify_success')
    @pytest.mark.asyncio
    async def test_completion_message_building(self, mock_update_slack, mock_build_completion, mock_extract_sheet):
        """Test that completion message is properly built with correct parameters"""
        
        # Mock dependencies
        mock_extract_sheet.return_value = "https://docs.google.com/spreadsheets/test"
        expected_completion_message = "✅ *Refund Processing Complete* - Open Registration restocked by joe"
    ...
    async def test_restock_with_direct_inventory_item_id(self):
        ...
        with patch.object(self.slack_refunds_utils, 'build_completion_message_after_restocking') as mock_build_message, \
             patch.object(self.slack_refunds_utils, 'update_slack_on_shopify_success') as mock_update_slack:
