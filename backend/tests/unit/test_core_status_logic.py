...
📋 Inventory restocking pending"""
        ...
        assert (
            "✅ *Order Not Canceled*, processed by <@U12345>" in message_text
        ), f"Original order processor attribution missing in: {message_text}"
        assert (
            "✅ *Not Refunded by <@U67890>*" in message_text
        ), f"Refund processor attribution missing in: {message_text}"
        assert "📋 Inventory restocking pending" in message_text

        # Count user attributions to ensure both are present
        user_attributions = [line for line in message_text.split("\n") if "@U" in line]
        assert (
            len(user_attributions) >= 2
        ), f"Expected at least 2 user attributions, got {len(user_attributions)}: {user_attributions}"
...
📋 Inventory restocking pending"""
    ...
    def test_no_duplicate_status_lines(self, message_builder):
        ...
        order_status_count = len(
            [line for line in lines if "Order Not Canceled" in line]
        )
        refund_pending_count = len(
            [line for line in lines if "📋 Refund processing pending" in line]
        )
        inventory_pending_count = len(
            [line for line in lines if "📋 Inventory restocking pending" in line]
        )

        # Each status should appear exactly once
        assert (
            order_status_count == 1
        ), f"Order status appears {order_status_count} times, should be 1"
        assert (
            refund_pending_count == 1
        ), f"Refund pending appears {refund_pending_count} times, should be 1"
        assert (
            inventory_pending_count == 1
        ), f"Inventory pending appears {inventory_pending_count} times, should be 1"

    def test_no_refund_preserves_user_attribution_in_all_paths(
        self, slack_utils, mock_orders_service
    ):
...
*Total Paid*: $50.00"""
        ...
        assert (
            "processed by <@U12345>" in message_text
        ), f"CRITICAL: Original order processor lost! Message: {message_text}"
        assert (
            "by <@U67890>" in message_text
        ), f"CRITICAL: Refund processor attribution lost! Message: {message_text}"
        ...
        assert (
            len(order_lines) == 1
        ), f"Expected exactly 1 order status line with U12345, got {len(order_lines)}: {order_lines}"
        assert (
            len(refund_lines) == 1
        ), f"Expected exactly 1 refund status line with U67890, got {len(refund_lines)}: {refund_lines}"
...
Restock Inventory?"""
        ...
        assert (
            "✅ *Inventory not restocked by <@U88888>*" in result
        ), f"CRITICAL: Inventory status not updated! Message: {result}"
        assert (
            "📋 Inventory restocking pending" not in result
        ), f"CRITICAL: Pending inventory status still present! Message: {result}"
        ...
        ), f"Expected exactly 1 inventory completion line, got {len(inventory_lines)}: {inventory_lines}"

        # Verify it starts with green check
        inventory_line = inventory_lines[0]
        assert inventory_line.startswith(
            "✅"
        ), f"Inventory status line should start with ✅, got: {inventory_line}"
...
Restock Inventory?"""
        ...
        assert (
            "✅ *Inventory restocked (Large) by <@U77777>*" in result
        ), f"CRITICAL: Restock status not updated! Message: {result}"
        assert (
            "📋 Inventory restocking pending" not in result
        ), f"CRITICAL: Pending inventory status still present! Message: {result}"
        ...
        inventory_line = inventory_lines[0]
        assert (
            "(Large)" in inventory_line
        ), f"Inventory status should include variant name, got: {inventory_line}"
        assert inventory_line.startswith(
            "✅"
        ), f"Inventory status line should start with ✅, got: {inventory_line}"
