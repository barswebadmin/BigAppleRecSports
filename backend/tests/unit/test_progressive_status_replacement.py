...
class TestProgressiveStatusReplacement:
    ...
    def test_order_cancellation_replacement_not_canceled(self, message_builder):
        ...
        status_lines = [line for line in message_text.split('\n') if "Order Not Canceled" in line]
        assert len(status_lines) == 1, f"Found duplicate order status lines: {status_lines}"
...
*Total Paid:* $50.00"""
        ...
        refund_completed_lines = [line for line in message_text.split('\n') if "Refund processing completed" in line]
        assert len(refund_completed_lines) == 1, f"Should have exactly 1 refund completion line, got: {refund_completed_lines}"
...
📋 Inventory restocking pending"""
    ...
    def test_no_duplicate_lines_comprehensive_flow(self, message_builder, slack_utils):
        ...
        order_status_lines = [line for line in final_message.split('\n') if "Order Not Canceled" in line]
        refund_status_lines = [line for line in final_message.split('\n') if "Refund processing completed" in line or "refund** issued by" in line]
        inventory_status_lines = [line for line in final_message.split('\n') if "Inventory restocked" in line]
        
        assert len(order_status_lines) == 1, f"Expected 1 order status line, got {len(order_status_lines)}: {order_status_lines}"
        assert len(inventory_status_lines) == 1, f"Expected 1 inventory status line, got {len(inventory_status_lines)}: {inventory_status_lines}"
