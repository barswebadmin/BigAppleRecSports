...
class TestSlackUpdateLogic:
    ...
    async def test_handle_edit_request_details_success(self):
        ...
        order_field = next(
            (b for b in blocks if b.get("block_id") == "order_number_input"), None
        )
        assert order_field is not None
        assert order_field["element"]["initial_value"] == "42234"  # Without #

        email_field = next(
            (b for b in blocks if b.get("block_id") == "requestor_email_input"), None
        )
    ...
    async def test_handle_edit_request_details_submission_success_validation(self):
        ...
        self.utils.update_slack_on_shopify_success = Mock(
        ...
        payload = {
            "view": {
                "state": {
                    "values": {
                        "order_number_input": {"order_number": {"value": "42235"}},
    ...
    async def test_handle_edit_request_details_submission_order_not_found(self):
        ...
        payload = {
            "view": {
                "state": {
                    "values": {
                        "order_number_input": {"order_number": {"value": "99999"}},
                        "requestor_email_input": {
        ...
        assert (
            "Order not found with this number" in result["errors"]["order_number_input"]
        )
