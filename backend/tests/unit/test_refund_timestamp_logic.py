...
class TestRefundTimestampLogic:
    ...
    def test_refund_calculation_defaults_to_current_time_when_no_timestamp_provided(self):
    ...
    def test_different_refund_tiers_based_on_submission_time(self):
        ...
        for case in test_cases:
            with patch('utils.date_utils.datetime') as mock_datetime:
                ...
                assert case["expected_description"] in result["message"], \
    ...
    def test_calculate_refund_amount_function_directly(self):
        ...
        submission_time = datetime(2024, 9, 24, 10, 0, 0, tzinfo=timezone.utc)  # 3 weeks before
    ...
    def test_logging_shows_correct_timestamps(self, mock_logger):
        ...
        season_start_logged = any("Season Start Date (UTC @ 7am):" in call for call in log_calls)
        submission_logged = any("Request Submitted At (UTC):" in call for call in log_calls)
        ...
        submission_log = next(call for call in log_calls if "Request Submitted At (UTC):" in call)
