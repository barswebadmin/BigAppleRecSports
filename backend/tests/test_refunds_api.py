...
class TestRefundsAPI:
    ...
    def test_successful_order_validation_and_slack_notification(
        self, mock_slack_service, mock_orders_service
    ):
    ...
    def test_requestor_name_variations(self):
        ...
        test_cases = [
            ...
            {"input": "John Doe", "expected": {"first": "John", "last": "Doe"}},
            {"input": "SingleName", "expected": {"first": "SingleName", "last": ""}},
        ]
    ...
    @patch("routers.refunds.orders_service", autospec=True)
    @patch("routers.refunds.slack_service", autospec=True)
    def test_orders_service_exception_handling(
        self, mock_slack_service, mock_orders_service
    ):
        """Test handling of exceptions from orders service"""

        # Mock orders service throwing an exception
        mock_orders_service.fetch_order_details_by_email_or_order_name.side_effect = (
    ...
    @patch("routers.refunds.orders_service", autospec=True)
    @patch("routers.refunds.slack_service", autospec=True)
    def test_slack_service_exception_handling(
        self, mock_slack_service, mock_orders_service
    ):
        ...
        mock_slack_service.send_refund_request_notification.side_effect = Exception(
            "Slack API error"
        )
...
class TestRefundsAPIIntegration:
    ...
    def test_full_success_flow_logging(self, mock_slack_service, mock_orders_service):
        ...
        with patch("routers.refunds.logger") as mock_logger:
            ...
            assert any(
                "Processing refund Slack notification for order 12345" in log
