...
✅ **Process Complete**"""
    ...
    def test_initial_success_message_format(self):
        ...
        message_builder = SlackMessageBuilder({"pickleball": "@sport_groups_pickleball"})
        
        with patch('services.slack.message_builder.format_date_and_time') as mock_format:
        ...
        assert "<https://admin.shopify.com/store/09fe59-3/customers/6875123456789|John Doe>" in message_text
    ...
    def test_after_order_decision_message_format(self, order_cancelled):
        ...
        assert message_text.startswith("*Request Type*:"), "Should start with request type"
    ...
    def test_after_process_refund_message_format(self):
        ...
        slack_utils.message_builder = SlackMessageBuilder({"pickleball": "@sport_groups_pickleball"})
        ...
        assert message_text.startswith("📧 *Requested by:*"), "Should start with requestor"
        ...
        assert len(result["action_buttons"]) >= 3  # At least 2 restock + 1 do not restock
    ...
    @pytest.mark.parametrize("restock_action,variant_name", [
        ("confirm_restock_early_bird", "Early Bird"),
        ("do_not_restock", "")
    ])
    def test_after_inventory_decision_message_format(self, restock_action, variant_name):
        ...
        # Test customer hyperlink persistence
        assert "<https://admin.shopify.com/store/09fe59-3/customers/6875123456789|John Doe>" in result
    ...
    def test_status_progression_logic(self):
        ...
        status_sequences = [
            ...
            [
                ("initial", ["📋 Order cancellation pending", "📋 Refund processing pending", "📋 Inventory restocking pending"]),
                ("after_cancel", ["✅ *Order Canceled*", "📋 Refund processing pending", "📋 Inventory restocking pending"]),
                ("after_refund", ["✅ *Order Canceled*", "✅ Refund processing completed", "📋 Inventory restocking pending"]),
                ("after_restock", ["✅ *Order Canceled*", "✅ Refund processing completed", "✅ *Inventory restocked"])
            ],
            # No refund path: cancel → no refund → restock
            [
                ("initial", ["📋 Order cancellation pending", "📋 Refund processing pending", "📋 Inventory restocking pending"]),
                ("after_cancel", ["✅ *Order Canceled*", "📋 Refund processing pending", "📋 Inventory restocking pending"]),
                ("after_no_refund", ["✅ *Order Canceled*", "✅ *Not Refunded", "📋 Inventory restocking pending"]),
                ("after_no_restock", ["✅ *Order Canceled*", "✅ *Not Refunded", "✅ *Inventory not restocked"])
            ]
        ]
