...
class TestComprehensiveE2EFlows:
    ...
    async def test_complete_flow_scenarios(self, scenario_name):
        ...
        with context_manager:
            ...
            for step in scenario["steps"][
                1:
            ]:  # Skip initial_request since we already did it
            ...
            for expected_status in scenario["expected_final_status"]:
                assert (
                    expected_status in final_message
                ), f"Missing final status: {expected_status}\nFull message: {final_message}"

            # Verify no pending indicators remain
            for pending_indicator in scenario["expected_no_pending"]:
                assert (
                    pending_indicator not in final_message
                ), f"Pending indicator still present: {pending_indicator}\nFull message: {final_message}"
    ...
    @pytest.mark.parametrize(
        "error_step,expected_behavior",
        [
            ("cancel_order_failure", "should show error and maintain current state"),
            (
                "refund_creation_failure",
                "should show error and maintain refund pending status",
            ),
            (
                "inventory_restock_failure",
                "should show error and maintain inventory pending status",
            ),
            (
                "slack_update_failure",
                "should log error but maintain internal state consistency",
            ),
        ],
    )
    @pytest.mark.asyncio
        ...
        with context_manager:
            ...
            try:
                if error_step == "cancel_order_failure":
                    message_state = await self._execute_step(
                        "cancel_order", message_state
                    )
                    ...
                    message_state = await self._execute_step(
                        "restock_inventory", message_state
                    )
    ...
    async def test_confirmation_modal_flows(self):
        ...
        with context_manager:
            ...
            await self._test_restock_confirmation_modal(
                message_state, "restock_early_bird"
            )
    ...
    def _mock_all_services(self):
        ...
        mock_orders_service.fetch_order_details_by_email_or_order_name.return_value = (
            self.mock_data["base_order"]
        )
        mock_orders_service.check_existing_refunds.return_value = self.mock_data[
            "success_responses"
        ]["no_refunds"]
        mock_orders_service.calculate_refund_due.return_value = self.mock_data[
            "success_responses"
        ]["refund_calculation"]
        ...
        mock_orders_service.create_refund_or_credit.return_value = {
            "success": True,
            "message": "Refund created",
        }

        mock_shopify_service = Mock()
        mock_shopify_service.adjust_inventory.return_value = {
            "success": True,
            "message": "Inventory adjusted",
        }
        mock_shopify_service.get_customer_by_email.return_value = self.mock_data[
            "success_responses"
        ]["customer_data"]

        mock_slack_service = Mock()
        mock_slack_service.send_refund_request_notification.return_value = (
            self.mock_data["success_responses"]["slack_message"]
        )
        ...
        patches = [
            ...
            patch(
                "services.slack.api_client.SlackApiClient.send_message",
                return_value={"ok": True, "ts": "1234567890.123456"},
            ),
            patch(
                "services.slack.api_client.SlackApiClient.update_message",
                return_value={"ok": True},
            ),
            patch(
                "services.slack.api_client.SlackApiClient.send_modal",
                return_value={"ok": True},
            ),
            # Mock Shopify requests to prevent real API calls
            patch(
                "services.shopify.shopify_service.ShopifyService._make_shopify_request",
    ...
    async def _execute_initial_request(self) -> Dict[str, Any]:
        ...
        with (
            ...
            mock_refunds.return_value = self.mock_data["success_responses"][
                "no_refunds"
            ]
            mock_calc.return_value = self.mock_data["success_responses"][
                "refund_calculation"
            ]
            mock_customer.return_value = self.mock_data["success_responses"][
                "customer_data"
            ]
            mock_slack.return_value = self.mock_data["success_responses"][
                "slack_message"
            ]
    ...
    async def _execute_step(
        self, step: str, message_state: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        if step == "cancel_order":
            ...
            return await self._execute_proceed_without_cancel(
                slack_service, message_state
            )
    ...
    async def _execute_cancel_order(
        self, slack_service: SlackService, message_state: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Execute cancel order step."""
        with (
            patch("services.orders.OrdersService.cancel_order") as mock_cancel,
            patch("services.slack.SlackService.update_slack_message") as mock_update,
        ):
            mock_cancel.return_value = self.mock_data["success_responses"][
                "cancel_order"
            ]
            mock_update.return_value = self.mock_data["success_responses"][
                "slack_update"
            ]
            ...
            updated_message = self._update_message_with_order_status(
                ...
                True,
                self.mock_data["user_ids"]["order_processor"],
            )
    ...
    async def _execute_proceed_without_cancel(
        self, slack_service: SlackService, message_state: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _execute_process_refund(
        self, slack_service: SlackService, message_state: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Execute process refund step."""
        with (
            patch(
                "services.orders.OrdersService.create_refund_or_credit"
            ) as mock_refund,
            patch(
                "services.orders.OrdersService.fetch_product_variants"
            ) as mock_variants,
            patch("services.slack.SlackService.update_slack_message") as mock_update,
        ):
            mock_refund.return_value = self.mock_data["success_responses"][
                "create_refund"
            ]
            mock_variants.return_value = self.mock_data["success_responses"][
                "variant_names"
            ]
            mock_update.return_value = self.mock_data["success_responses"][
                "slack_update"
            ...
            request_data = {
                ...
                "refundType": "refund",
                "orderCancelled": str(
                    message_state.get("order_cancelled", False)
                ).lower(),
                "first": "John",
                "last": "Doe",
                "email": "john.doe@example.com",
            ...
            updated_message = self._update_message_with_refund_status(
                ...
                95.00,
                "refund",
                self.mock_data["user_ids"]["refund_processor"],
            )
    ...
    async def _execute_custom_refund(
        self, slack_service: SlackService, message_state: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _execute_restock_inventory(
        self, slack_service: SlackService, message_state: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Execute restock inventory step."""
        with (
            patch("services.shopify.ShopifyService.adjust_inventory") as mock_restock,
            patch("services.slack.SlackService.update_slack_message") as mock_update,
        ):
            mock_restock.return_value = self.mock_data["success_responses"][
                "restock_inventory"
            ]
            mock_update.return_value = self.mock_data["success_responses"][
                "slack_update"
            ...
            updated_message = self._update_message_with_inventory_status(
                ...
                True,
                "Early Bird",
                self.mock_data["user_ids"]["inventory_processor"],
            )
    ...
    async def _execute_do_not_restock(
        self, slack_service: SlackService, message_state: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _execute_deny_request(
        self, slack_service: SlackService, message_state: Dict[str, Any]
    ) -> Dict[str, Any]:
...
📝 *Notes:* Customer needs refund due to scheduling conflict"""
    ...
    def _update_message_with_order_status(
        self, current_message: str, cancelled: bool, user_id: str
    ) -> str:
    ...
    def _update_message_with_refund_status(
        self,
        current_message: str,
        refunded: bool,
        amount: float,
    ...
    def _update_message_with_inventory_status(
        self, current_message: str, restocked: bool, variant_name: str, user_id: str
    ) -> str:
        ...
        updated = current_message.replace(
            "📋 Inventory restocking pending", status_line
        )
        return updated

    def _update_message_with_denial_status(
...
✅ **Process Complete**"""
    ...
    def _validate_message_state_consistency(
        self, message_state: Dict[str, Any], step: str
    ):
        ...
        assert (
            "<https://admin.shopify.com/store/test/customers/" in message
        ), f"Customer hyperlink missing after {step}"

        # Order number should always be present and hyperlinked
        assert (
            "<https://admin.shopify.com/store/test/orders/" in message
        ), f"Order hyperlink missing after {step}"
        ...
        if step not in ["final", "after_restock", "after_no_restock"]:
            assert (
                "*Request Submitted At:*" in message
            ), f"Request timestamp missing after {step}"
    ...
    def _setup_error_condition(self, mocks, error_step):
        ...
        if error_step == "cancel_order_failure":
            ...
            mocks["shopify_service"].adjust_inventory.return_value = {
                "success": False,
    ...
    def _setup_initial_error_state(self, mocks, initial_state):
        ...
        if initial_state == "email_mismatch":
            mocks[
                ...
                "success": True,
                "data": {
                    **self.mock_data["base_order"]["data"],
    ...
    def _validate_message_format_and_block_order(self, message: str, step_name: str):
        ...
        for element in core_elements:
            assert (
                element in message
            ), f"Missing core element '{element}' in {step_name} message"
    ...
    def _validate_hyperlink_formats(self, message: str, step_name: str):
        ...
        if step_name != "initial":
            customer_pattern = (
                r"📧 \*Requested by:\* <https://admin\.shopify\.com/store/[^|]+\|[^>]+>"
            )
            assert re.search(
                customer_pattern, message
            ), f"Customer hyperlink format incorrect in {step_name}"

        # Order hyperlink format (all messages)
        order_pattern = (
            r"\*Order Number\*: <https://admin\.shopify\.com/store/[^|]+\|#[^>]+>"
        )
        assert re.search(
            order_pattern, message
        ), f"Order hyperlink format incorrect in {step_name}"

        # Product hyperlink format (all messages)
        product_pattern = (
            r"\*Product Title:\* <https://admin\.shopify\.com/store/[^|]+\|[^>]+>"
        )
        assert re.search(
            product_pattern, message
        ), f"Product hyperlink format incorrect in {step_name}"

        # Google Sheets link format (all messages)
        sheets_pattern = r"🔗 \*<https://docs\.google\.com/spreadsheets/[^|]+\|[^>]+>\*"
        assert re.search(
            sheets_pattern, message
        ), f"Google Sheets link format incorrect in {step_name}"
    ...
    def _validate_status_indicators_by_step(self, message: str, step_name: str):
        ...
        if step_name == "initial":
            ...
            assert (
                "✅ *Order Canceled*" in message or "✅ *Order Not Canceled*" in message
            )
            assert "📋 Refund processing pending" in message
            assert "📋 Inventory restocking pending" in message
            ...
            assert (
                "✅ Refund processing completed" in message
                or "✅ *Not Refunded" in message
            )
            assert "📋 Inventory restocking pending" in message
            ...
            assert (
                "✅ *Inventory restocked" in message
                or "✅ *Inventory not restocked" in message
            )
    ...
    def _validate_step_specific_elements(self, message: str, step_name: str):
        ...
        if step_name == "initial":
            ...
            assert re.search(
                refund_pattern, message
            ), f"Refund footer missing in {step_name}"
            ...
            assert re.search(
                waitlist_pattern, message
            ), f"Waitlist link missing in {step_name}"
    ...
    def _validate_no_duplicate_status_lines(self, message: str):
        ...
        order_statuses = [
            ...
            if ("Order Canceled" in line or "Order Not Canceled" in line)
        ]
        refund_statuses = [
            line
            for line in status_lines
            if ("Refund" in line or "refund" in line or "Not Refunded" in line)
        ]
        ...
        assert (
            len(refund_statuses) <= 1
        ), f"Duplicate refund statuses: {refund_statuses}"
        assert (
            len(inventory_statuses) <= 1
        ), f"Duplicate inventory statuses: {inventory_statuses}"
