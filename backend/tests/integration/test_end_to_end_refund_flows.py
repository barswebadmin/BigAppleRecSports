...
class TestEndToEndRefundFlows:
    ...
    def _setup_shopify_mocks(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "variant_names": [
                {"name": "Early Bird", "gid": "gid://shopify/ProductVariant/43691235926110"},
                {"name": "Regular", "gid": "gid://shopify/ProductVariant/43691235926111"}
            ],
    ...
    @patch('services.slack.SlackService.send_refund_request_notification')
    async def test_duplicate_refund_pending_flow(self, mock_slack, mock_refunds, mock_fetch):
        """
    ...
    @patch('routers.refunds.slack_service.send_refund_request_notification')
    async def test_valid_refund_request_early_timing(self, mock_slack, mock_customer, mock_calc, mock_refunds, mock_fetch):
        """
    ...
    @patch('routers.refunds.orders_service.shopify_service.get_customer_by_email')
    @patch('routers.refunds.slack_service.send_refund_request_notification')
    async def test_valid_credit_request_early_timing(self, mock_slack, mock_customer, mock_calc, mock_refunds, mock_fetch):
        """
    ...
    @pytest.mark.parametrize("submission_date,expected_amount,expected_tier", [
        ("2024-09-15T15:30:00Z", 95.00, "more than 2 weeks before"),  # >2 weeks before
        ("2024-10-10T15:30:00Z", 90.00, "before week 1 started"),     # <2 weeks, before start
        ("2024-10-18T15:30:00Z", 80.00, "after the start of week 1"), # Week 1 started
        ("2024-10-25T15:30:00Z", 70.00, "after the start of week 2"), # Week 2 started
        ("2024-11-01T15:30:00Z", 60.00, "after the start of week 3"), # Week 3 started
        ("2024-11-08T15:30:00Z", 50.00, "after the start of week 4"), # Week 4 started
        ("2024-11-20T15:30:00Z", 0.00, "after week 5 had already started"), # Too late
    ])
    @pytest.mark.asyncio
    async def test_refund_timing_tiers(self, submission_date, expected_amount, expected_tier):
        """
    ...
    async def test_order_cancellation_failure(self, mock_update, mock_cancel):
        ...
        """
        # Arrange
        mock_cancel.return_value = {"success": False, "message": "Order cancellation failed"}
        mock_update.return_value = self.mock_slack_responses["message_updated"]
    ...
    async def test_refund_creation_failure(self, mock_update, mock_refund):
        ...
        """
        # Arrange
        mock_refund.return_value = {"success": False, "message": "Refund creation failed"}
        mock_update.return_value = self.mock_slack_responses["message_updated"]
    ...
    @patch('routers.refunds.orders_service.shopify_service.get_customer_by_email')
    @patch('services.slack.SlackService.update_slack_message')
    async def test_edit_request_details_success_submission(self, mock_update, mock_customer, mock_calc, mock_refunds, mock_fetch):
        """
        ...
        # Act
        result = await slack_service.handle_edit_request_details_submission(modal_payload)
    ...
    def test_message_format_consistency(self):
        ...
        for template_name, template_content in self.expected_message_templates.items():
            ...
            for element in required_elements:
                if element == "Product:" and ("email_mismatch" in template_name or "duplicate_refund" in template_name):
                    # Email mismatch and duplicate refund might not have product info if validation failed early
                    continue
                if element == "Order Date:" and ("email_mismatch" in template_name or "duplicate_refund" in template_name):
                    # Email mismatch and duplicate refund might not have order date if validation failed early
                    continue
                if element == "Original Amount Paid:" and ("email_mismatch" in template_name or "duplicate_refund" in template_name):
                    # Email mismatch and duplicate refund might not have amount if validation failed early
                    continue
                    
                assert element in template_content, f"Template '{template_name}' missing '{element}'"
            
            # Check for hyperlink formatting
            if template_name not in ["order_not_found", "email_mismatch", "duplicate_refund_pending"]:
                assert "<https://admin.shopify.com" in template_content, f"Template '{template_name}' missing Shopify links"
                
                # User tags only appear in messages after user actions (not initial messages)
                if not template_name.startswith("success_initial"):
                    assert "<@U" in template_content, f"Template '{template_name}' missing user tags"
