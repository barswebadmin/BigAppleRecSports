...
────────────
    ...
    SHOPIFY CLIENT (transport + execution only)
        ...
        │
        └── ShopifyClient.run_tags_add_batch(pairs)  — batched ``tagsAdd`` (aliased fields)
...
# Max ``tagsAdd`` calls (aliases) per GraphQL mutation document.
TAGS_ADD_BATCH_SIZE = 20
...
class QueryOp:
    ...
    def search_string(self, kwargs: dict[str, Any]) -> str:
        """Build a Shopify search string from caller kwargs.

        Scalar values become ``field:value`` clauses joined with `` AND ``.
        Keys are NOT camelCased — Shopify search uses snake_case (e.g.
        ``product_id:``, ``created_at:``, ``email:``).

        Sequence values for one field become ``(f:a OR f:b OR …)`` so callers
        can batch-match (e.g. multiple emails in one ``customers`` query).
        """
        clauses: list[str] = []
        for n, v in self.variables.items():
            ...
            if isinstance(val, (list, tuple)):
                if not val:
                    continue
                parts = [
                    f"{key}:{ResourceId.of(v.gid, item).digits if v.gid else item}" for item in val
                ]
                clauses.append("(" + " OR ".join(parts) + ")" if len(parts) > 1 else parts[0])
            else:
                clauses.append(f"{key}:{ResourceId.of(v.gid, val).digits if v.gid else val}")
...
class ShopifyClient:
    ...
    def run_tags_add_batch(
        self,
        pairs: list[tuple[str, list[str]]],
        *,
        dry_run: bool = False,
    ) -> list[Any]:
        """Run multiple ``tagsAdd`` mutations in one GraphQL request (aliases).

        Each ``pairs`` row is ``(customer_id, tags)``. ``customer_id`` may be a
        GID or bare Shopify id. Returns one Box per row (``user_errors`` only),
        same order as ``pairs``.
        """
        if not pairs:
            return []
        ...
        n = len(pairs)
        var_defs = DSLVariableDefinitions()
        field_nodes: list[DSLField] = []
        wire: dict[str, Any] = {}
        for i, (raw_id, tags) in enumerate(pairs):
            cid = raw_id if isinstance(raw_id, str) else str(raw_id)
            cid = cid.strip()
            if not cid.startswith("gid://"):
                cid = ResourceId.of("customer", cid).gid
            wire[f"id{i}"] = cid
            wire[f"tags{i}"] = list(tags)
            payload = ds.TagsAddPayload
            field_nodes.append(
                ds.Mutation.tagsAdd.alias(f"t{i}")
                .args(id=getattr(var_defs, f"id{i}"), tags=getattr(var_defs, f"tags{i}"))
                .select(payload.userErrors.select(ds.UserError.field, ds.UserError.message))
            )
        mut = DSLMutation(*field_nodes)
        mut.variable_definitions = var_defs
        req = dsl_gql(mut)
        raw = self.execute(req, wire, dry_run=dry_run)
        if dry_run:
            return []
        return [self.boxify((raw or {}).get(f"t{i}") or {}) for i in range(n)]
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        while True:
            ...
            print(f"  page {page_num}: +{len(nodes)} (total {len(all_nodes)})", flush=True)
