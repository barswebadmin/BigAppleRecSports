...
# Compatibility shims — Stage 3 moved the canonical implementations to
# ``modules.refunds.services.shopify_refund_service.ShopifyRefundService``.
# The thin wrappers below preserve the legacy module's external surface and
# convert ``ShopifyUserError`` (raised from the new helpers) into the legacy
# ``UnprocessableError`` so callers in this file (the orchestrator below)
# behave identically. Stage 5 retires the entire legacy module.
def _parent_capture_txn(transactions: list[dict]) -> dict | None:
    ...
    return ShopifyRefundService._parent_capture_txn(transactions)
...
def _build_refund_transactions_for_shopify(
    ...
    try:
        return ShopifyRefundService._build_refund_transactions_for_shopify(
...
def _build_store_credit_refund_methods(amount: float, currency: str) -> list[dict]:
    ...
    return ShopifyRefundService._build_store_credit_refund_methods(amount, currency)
...
async def execute_refund_create(body: RefundExecuteRequest) -> dict[str, Any]:
    ...
    Stage 3 migration: delegates Shopify mutations to
    ``ShopifyRefundService`` (D31). ``ShopifyUserError`` is converted to
    the legacy ``UnprocessableError`` so existing callers (controllers /
    routes) continue to see the same exception type until Stage 5 retires
    this orchestrator.
    """
    ...
    refund_service = ShopifyRefundService(client)
    out: dict[str, Any] = {"cancel": None, "refund": None}

    if body.cancel_order:
        try:
            out["cancel"] = await refund_service.cancel_order(
    ...
    try:
        out["refund"] = await refund_service.create_refund(
            order_id=body.order_id,
...
# ``ShopifyRefundService.create_refund`` supersedes it. A repo grep
# confirmed no other consumer (no imports of
# ``services.refunds.service.create`` anywhere). See progress-stage-3.md.
