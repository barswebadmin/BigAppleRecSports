function schedulePriceChanges(rowNumber = null) {
  ...
  const scheduledColIndex = sheetHeaders.findIndex(h => h.toLowerCase().includes("price changes scheduled?"));
  ...
  try {
    ...
    if (response.getResponseCode() === 200) {
      sheet.getRange(rowNumber, scheduledColIndex).setValue(true);
