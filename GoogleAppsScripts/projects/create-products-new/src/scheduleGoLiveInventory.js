...
function promptAndScheduleGoLiveInventory(productUrl, variantIds, registrationDates, totalInventory, rowObject) {
  ...
  const division = rowObject?.division || 'Open';
  ...
  // Check if TNB/WTNB and BIPOC are combined (same variant ID and same date)
  const areCombined = wtnbVariantId && bipocVariantId && 
                      wtnbVariantId === bipocVariantId &&
                      wtnbRegistrationStartDateTime?.raw && 
                      bipocRegistrationStartDateTime?.raw &&
                      new Date(wtnbRegistrationStartDateTime.raw).getTime() === new Date(bipocRegistrationStartDateTime.raw).getTime();
  
  if (areCombined) {
    // Combined variant - add only once
    registrations.push({
      type: `${division === 'Open' ? 'W' : ''}TNB+ and BIPOC Early`,
      time: wtnbRegistrationStartDateTime,
      variantId: wtnbVariantId
    });
  } else {
    // Separate variants - add each if they exist
  ...
  // Map registration type to variant type for tag management
  const variantTypeMap = {
    'Veteran': 'vet',
    'WTNB+ Early': 'wtnb',
    'TNB/WTNB Early': 'wtnb', // Alternative spelling
    'BIPOC Early': 'bipoc',
    'Early': 'early',
    'Open': 'open'
  };
  
  // Handle combined TNB+ and BIPOC variant types (both with and without W prefix)
  if (firstRegType && firstRegType.includes('TNB+') && firstRegType.includes('BIPOC')) {
    variantTypeMap[firstRegType] = 'wtnb'; // Use wtnb as the type for combined variant
  }
  
  const firstVariantType = variantTypeMap[firstRegType] || null;
...
function scheduleInitialInventoryAddition(productUrl, variantGid, scheduledAt, numberVetSpotsToReleaseAtGoLive, totalInventory, variantType, rowObject) {
  ...
  // Add variantType if available (for tag management)
  if (variantType) {
...
function scheduleGoLiveInventoryFromRow() {
  ...
  const formatDateTime = (dateValue) => {
    ...
    try {
      ...
      // Add 1 minute to account for lambda's wait_until_next_minute() delay
      const dateWithDelay = new Date(date);
      dateWithDelay.setMinutes(dateWithDelay.getMinutes() + 1);
      ...
      const formatted = dateWithDelay.toLocaleString('en-US', options).replace(',', '');
      
      return {
        raw: date, // Keep raw as original (for scheduling), formatted shows actual execution time
        formatted: formatted
      };
    } catch (err) {
