function scheduleInventoryMoves(selectedRow = null) {
  ...
  Logger.log(`Column indices - vet: ${vetVariantColIndex}, wtnb: ${wtnbVariantColIndex}, bipoc: ${bipocVariantColIndex}, early: ${earlyVariantColIndex}, open: ${openVariantColIndex}`);
  ...
  // Debug: Log raw cell values before extraction
  Logger.log(`Raw cell values - vet: "${rowData[vetVariantColIndex]}", wtnb: "${rowData[wtnbVariantColIndex]}", bipoc: "${rowData[bipocVariantColIndex]}", open: "${rowData[openVariantColIndex]}"`);
  ...
  Logger.log(`Raw date objects - vet: ${JSON.stringify(vetDateTime)}, wtnb: ${JSON.stringify(wtnbDateTime)}, bipoc: ${JSON.stringify(bipocDateTime)}`);

  // Build chronological array of all variants
  const variants = [];
  
  if (vetGid && vetDateString) {
    Logger.log(`✅ Adding vet variant: ${vetGid} with date ${vetDateString}`);
    variants.push({
      type: 'vet',
    ...
    Logger.log(`⏭️ Skipping vet variant - gid: ${vetGid}, date: ${vetDateString}`);
  }
  
  // Handle TNB/WTNB and BIPOC variants
  if (wtnbGid && wtnbDateString) {
    Logger.log(`✅ Adding wtnb variant: ${wtnbGid} with date ${wtnbDateString}`);
    variants.push({
      type: 'wtnb',
    ...
    Logger.log(`⏭️ Skipping wtnb variant - gid: ${wtnbGid}, date: ${wtnbDateString}`);
  }
  
  if (bipocGid && bipocDateString) {
    Logger.log(`✅ Adding bipoc variant: ${bipocGid} with date ${bipocDateString}`);
    variants.push({
      type: 'bipoc',
    ...
    Logger.log(`⏭️ Skipping bipoc variant - gid: ${bipocGid}, date: ${bipocDateString}`);
  }
  ...
  variants.sort((a, b) => {
    ...
    Logger.log(`Sorting: ${a.type} (${dateA}) vs ${b.type} (${dateB}) = ${result}`);
    return result;
  });
  
  Logger.log(`Chronological variants (${variants.length} total): ${JSON.stringify(variants.map(v => ({type: v.type, name: v.name, date: v.date, gid: v.gid})), null, 2)}`);

  if (variants.length < 2) {
    Logger.log(`❌ Not enough variants to create moves. Found ${variants.length} variants.`);
    SpreadsheetApp.getUi().alert(`❌ Error: Need at least 2 variants to schedule moves. Found ${variants.length} variants with valid GIDs and dates.`);
    return;
  }
  ...
  for (let i = 0; i < variants.length - 1; i++) {
    ...
    Logger.log(`Creating movement ${i + 1}/${variants.length - 1}: ${source.type} → ${dest.type} (${source.name} → ${dest.name})`);
    requests.push({
