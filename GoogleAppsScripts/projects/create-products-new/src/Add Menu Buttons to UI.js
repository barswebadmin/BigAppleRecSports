...
function parseRowData(row, rowIndex) {
  ...
  // Track which headers were found
  ...
  if (missingHeaders.length > 0) {
    const missingHeaderNames = missingHeaders.map(key => {
      // Find the original header name from sheetHeaders
...
function createProduct() {
  ...
  // Helper function to add 1 minute to date (lambda waits until next minute before executing)
  const formatDateAndTimeWithLambdaDelay = (date) => {
    if (!date) return null;
    const d = new Date(date);
    d.setMinutes(d.getMinutes() + 1); // Add 1 minute to account for lambda's wait_until_next_minute()
  ...
  if (!rowObject['earlyRegistrationStartDateTime'] && rowObject['tnbWtnbRegistrationStartDateTime']) {
    // If using TNB/WTNB as early, use its formatted value (already has +1 minute)
