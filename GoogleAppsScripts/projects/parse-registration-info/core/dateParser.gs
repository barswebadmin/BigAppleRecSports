...
function parseFlexible_(raw, opts) {
  ...
  // 3. Month name formats: "October 12", "Oct 12, 2025", "Sept 18th 7PM"
  const monthNames = {
  ...
  // Enhanced pattern to handle ordinal numbers (1st, 2nd, 3rd, 4th) and optional time
  m = input.match(/^(\w+)\s+(\d{1,2})(?:st|nd|rd|th)?(?:,?\s*(\d{4}))?\s*(.+)?/i);
  if (m) {
    const monthName = m[1].toLowerCase();
    const day = parseInt(m[2]);
    const year = m[3] ? parseInt(m[3]) : currentYear;
    const timeStr = m[4]; // Optional time part
    ...
    if (monthNames.hasOwnProperty(monthName)) {
      ...
      // Parse time component if present and requested
      if (timeStr && opts?.assumeDateTime) {
        ...
        if (timeMatch) {
          ...
          if (meridiem === 'pm' && hour !== 12) hour += 12;
          if (meridiem === 'am' && hour === 12) hour = 0;
          // If no meridiem specified but hour <= 12, assume PM for evening times
          if (!meridiem && hour >= 5 && hour <= 12) hour += 12;
  ...
  // 4. Try alternative month formats with time: "Sept 18 7PM", "Mar 15th 8:00"
  m = input.match(/^(\w+)\s+(\d{1,2})(?:st|nd|rd|th)?\s+(\d{1,2})(?::(\d{2}))?\s*(am|pm)?/i);
  if (m) {
    const monthName = m[1].toLowerCase();
    const day = parseInt(m[2]);
  ...
  // 5. Try native Date parsing as fallback
