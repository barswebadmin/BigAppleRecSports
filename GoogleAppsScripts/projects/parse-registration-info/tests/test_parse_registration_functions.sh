...
run_test "parseSourceRowEnhanced_ function exists" \
    "grep -q 'function parseSourceRowEnhanced_' '$PROJECT_DIR/src/core/rowParser.gs'"

run_test "parseBFlags_ function exists" \
    "grep -q 'function parseBFlags_' '$PROJECT_DIR/src/core/flagsParser.gs'"

run_test "parseTimeRangeBothSessions_ function exists" \
    "grep -q 'function parseTimeRangeBothSessions_' '$PROJECT_DIR/src/core/timeParser.gs'"

run_test "parsePriceNumber_ function exists" \
    "grep -q 'function parsePriceNumber_' '$PROJECT_DIR/src/core/priceParser.gs'"

run_test "parseDateFlexibleDateOnly_ function exists" \
    "grep -q 'function parseDateFlexibleDateOnly_' '$PROJECT_DIR/src/core/dateParser.gs'"

run_test "parseDateFlexibleDateTime_ function exists" \
    "grep -q 'function parseDateFlexibleDateTime_' '$PROJECT_DIR/src/core/dateParser.gs'"

# Test 4: Migration functions exist
run_test "migrateRowToTarget_ function exists" \
    "grep -q 'function migrateRowToTarget_' '$PROJECT_DIR/src/core/migration.gs'"
...
run_test "TARGET_SPREADSHEET_ID constant exists" \
    "grep -q 'TARGET_SPREADSHEET_ID' '$PROJECT_DIR/src/config/constants.gs'"

run_test "CANONICAL_LOCATIONS array exists and has locations" \
    "grep -A 10 'CANONICAL_LOCATIONS' '$PROJECT_DIR/src/config/constants.gs' | grep -q 'Elliott Center\\|Chelsea Park'"

run_test "headerMapping object exists" \
    "grep -q 'headerMapping.*=' '$PROJECT_DIR/src/config/constants.gs'"

run_test "REQUIRED_TARGET_HEADERS array exists" \
    "grep -q 'REQUIRED_TARGET_HEADERS' '$PROJECT_DIR/src/config/constants.gs'"
...
run_test "toTitleCase_ helper exists" \
    "grep -q 'toTitleCase_\\|function.*toTitleCase' '$PROJECT_DIR/src/helpers/textUtils.gs'"
...
run_test "parseFlexible_ function exists" \
    "grep -q 'function parseFlexible_' '$PROJECT_DIR/src/core/dateParser.gs'"
...
run_test "Price parsing handles currency symbols" \
    "grep -A 5 'function parsePriceNumber_' '$PROJECT_DIR/src/core/priceParser.gs' | grep -q '\\$\\|parseFloat'"

# Test 9: Notes and special date parsing
run_test "parseNotes_ function exists for extracting special dates" \
    "grep -q 'function parseNotes_' '$PROJECT_DIR/src/core/notesParser.gs'"

run_test "Notes parsing extracts orientation and scout night" \
    "grep -A 20 'function parseNotes_' '$PROJECT_DIR/src/core/notesParser.gs' | grep -q 'orientation\\|scout'"

# Test 10: Error handling and unresolved tracking
run_test "Functions track unresolved items for debugging" \
    "grep -r 'unresolved\\.push\\|unresolved\\[' '$PROJECT_DIR/src/core/' | wc -l | grep -q '[1-9]'"

run_test "Required fields validation includes sport, day, price" \
    "grep -A 15 'REQUIRED_TARGET_HEADERS' '$PROJECT_DIR/src/config/constants.gs' | grep -q 'sport' && \
     grep -A 15 'REQUIRED_TARGET_HEADERS' '$PROJECT_DIR/src/config/constants.gs' | grep -q 'day' && \
     grep -A 15 'REQUIRED_TARGET_HEADERS' '$PROJECT_DIR/src/config/constants.gs' | grep -q 'price'"
...
run_test "Header mapping includes essential fields" \
    "grep -A 30 'headerMapping.*=' '$PROJECT_DIR/src/config/constants.gs' | grep -q 'sport' && \
     grep -A 30 'headerMapping.*=' '$PROJECT_DIR/src/config/constants.gs' | grep -q 'day' && \
     grep -A 30 'headerMapping.*=' '$PROJECT_DIR/src/config/constants.gs' | grep -q 'price' && \
     grep -A 30 'headerMapping.*=' '$PROJECT_DIR/src/config/constants.gs' | grep -q 'location'"

run_test "Target spreadsheet configuration is valid" \
    "grep 'TARGET_SPREADSHEET_ID' '$PROJECT_DIR/src/config/constants.gs' | grep -q '1w9Hj4JMmjTIQM5c8FbXuKnTMjVOLipgXaC6WqeSV_vc'"

# Test 13: Time parsing functionality
run_test "Time parsing handles multiple session formats" \
    "grep -A 10 'parseTimeRangeBothSessions_' '$PROJECT_DIR/src/core/timeParser.gs' | grep -q 'primary\\|alt\\|Start\\|End'"

run_test "Time parsing creates proper Date objects" \
    "grep -A 10 'parseTimeRangeBothSessions_' '$PROJECT_DIR/src/core/timeParser.gs' | grep -q 'Date\\|DateOnly'"

# Test 14: Division and sport category parsing
run_test "parseBFlags extracts division information" \
    "grep -A 10 'function parseBFlags_' '$PROJECT_DIR/src/core/flagsParser.gs' | grep -q 'division'"
