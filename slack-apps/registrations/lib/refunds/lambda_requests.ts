...
import type { PreparedRequest } from "../clients/prepared_request.ts";
import type { RefundTransaction } from "../../types/evaluation_payload.ts";
import { REFUND_PROCESS_URL } from "../../config.ts";

export type RefundType = "refund_to_original" | "store_credit";
 ...
 * credit). The Lambda routes on `refund_type` and uses `transactions` to build
