...
def validate_season_dates(product_gid, season_start_date, off_dates_comma_separated):
    ...
    extracted_start_date, extracted_off_dates = extract_season_dates(
    ...
    input_off_dates = (
        [
            format_date_only(d.strip())
            for d in off_dates_comma_separated.split(",")
            if d.strip()
        ]
        if off_dates_comma_separated
        else []
    )

    extracted_off_list = (
        [d.strip() for d in extracted_off_dates.split(",") if d.strip()]
        if extracted_off_dates
        else []
    )
