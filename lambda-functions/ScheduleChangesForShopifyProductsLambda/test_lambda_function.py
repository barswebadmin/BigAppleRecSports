...
class TestScheduleChangesForShopifyProductsLambda:
    ...
    def test_missing_action_type(self):
        ...
        event = {
            "body": json.dumps(
                {"scheduleName": "test-schedule", "groupName": "test-group"}
            )
        }
    ...
    def test_unsupported_action_type(self):
        ...
        event = {
            "body": json.dumps(
                {"actionType": "unsupported-action", "scheduleName": "test-schedule"}
            )
        }
    ...
    def test_inventory_movement_success(self, mock_create_inventory):
        ...
        mock_create_inventory.return_value = {
            ...
            "aws_response": {
                "ScheduleArn": "arn:aws:scheduler:us-east-1:123456789012:schedule/test-group/test-inventory-move"
            },
        }
        ...
        assert (
            "Schedule 'test-inventory-move' created successfully!"
            in response_body["data"]["message"]
        )
    ...
    def test_price_change_success(self, mock_create_price):
        ...
        assert response_body["success"] is True
        assert (
            "All price change schedules updated successfully!"
            in response_body["data"]["message"]
        )
    ...
    @patch(
        "inventory_addition_scheduler.create_initial_inventory_addition_and_title_change"
    )
    def test_inventory_addition_and_title_change_success(self, mock_create_addition):
        ...
        mock_create_addition.return_value = {
            ...
            "aws_response": {
                "ScheduleArn": "arn:aws:scheduler:us-east-1:123456789012:schedule/set-product-live/auto-set-123456789-kb-monday-socialdiv-live"
            },
        }
        ...
        assert response_body["success"] is True
        assert (
            "Schedule 'auto-set-123456789-kb-monday-socialdiv-live' created successfully!"
            in response_body["data"]["message"]
        )
        ...
        assert (
            call_args["actionType"]
            == "create-initial-inventory-addition-and-title-change"
        )
        assert (
            call_args["scheduleName"] == "auto-set-123456789-kb-monday-socialdiv-live"
        )
        assert (
            call_args["productTitle"]
            == "Big Apple Kickball - Monday - Social Division - Fall 2024"
        )
        assert call_args["inventoryToAdd"] == 180

    @patch("inventory_addition_scheduler.create_remaining_inventory_addition_schedule")
    def test_remaining_inventory_addition_success(self, mock_create_remaining):
        """Test successful remaining inventory addition scheduling to addRemainingInventoryToLiveProduct Lambda"""
        # Mock the remaining inventory scheduler response
        mock_create_remaining.return_value = {
            "message": "✅ Remaining inventory schedule 'auto-add-remaining-inventory-123456789-pb-tuesday-openDiv' created successfully!",
            "new_expression": "at(2024-01-01T10:00:00)",
            "lambda_input": {
                "productUrl": "https://09fe59-3.myshopify.com/admin/products/123456789",
                "productTitle": "Big Apple Pickleball - Tuesday - Open Division - Fall 2024",
                "variantGid": "gid://shopify/ProductVariant/111111111",
                "inventoryToAdd": 60,
            },
            "aws_response": {
                "ScheduleArn": "arn:aws:scheduler:us-east-1:123456789012:schedule/add-remaining-inventory-to-live-product/auto-add-remaining-inventory-123456789-pb-tuesday-openDiv"
            },
        }
        ...
        event = {
            "body": json.dumps(
                {
                    "actionType": "create-initial-inventory-addition-and-title-change",
                    "scheduleName": "auto-add-remaining-inventory-123456789-pb-tuesday-openDiv",
        ...
        assert response_body["success"] is True
        assert (
            "Remaining inventory schedule 'auto-add-remaining-inventory-123456789-pb-tuesday-openDiv' created successfully!"
            in response_body["data"]["message"]
        )

        # Verify the remaining inventory scheduler was called with correct parameters
        mock_create_remaining.assert_called_once()
        call_args = mock_create_remaining.call_args[0][0]
        assert (
            call_args["actionType"]
            == "create-initial-inventory-addition-and-title-change"
        )
        assert (
            call_args["scheduleName"]
            == "auto-add-remaining-inventory-123456789-pb-tuesday-openDiv"
        )
        assert call_args["groupName"] == "add-remaining-inventory-to-live-product"
        assert call_args["inventoryToAdd"] == 60

    @patch(
        "inventory_addition_scheduler.create_initial_inventory_addition_and_title_change"
    )
    @patch("inventory_addition_scheduler.create_remaining_inventory_addition_schedule")
    def test_groupname_determines_function_routing(
        self, mock_create_remaining, mock_create_initial
    ):
        """Test that groupName determines which function is called for create-initial-inventory-addition-and-title-change"""
        # Set up mock responses
        mock_create_initial.return_value = {
            "message": "Initial inventory schedule created"
        }
        mock_create_remaining.return_value = {
            "message": "Remaining inventory schedule created"
        }

        # Test case 1: add-remaining-inventory-to-live-product -> remaining function
        event_remaining = {
            "body": json.dumps(
                {
                    "actionType": "create-initial-inventory-addition-and-title-change",
                    "scheduleName": "test-remaining",
                    "groupName": "add-remaining-inventory-to-live-product",
        ...
        result = lambda_handler(event_remaining, {})
        assert result["statusCode"] == 201
        mock_create_remaining.assert_called_once()
        mock_create_initial.assert_not_called()

        # Reset mocks
        mock_create_remaining.reset_mock()
        mock_create_initial.reset_mock()

        # Test case 2: set-product-live -> initial function
        event_initial = {
            "body": json.dumps(
                {
                    "actionType": "create-initial-inventory-addition-and-title-change",
                    "scheduleName": "test-initial",
                    "groupName": "set-product-live",
                    "productUrl": "https://example.com/product/123",
                    "productTitle": "Test Product",
                    "variantGid": "gid://shopify/ProductVariant/123",
        ...
        result = lambda_handler(event_initial, {})
        assert result["statusCode"] == 201
        mock_create_initial.assert_called_once()
        mock_create_remaining.assert_not_called()
    ...
    def test_inventory_movement_validation_error(self, mock_create_inventory):
        ...
        mock_create_inventory.side_effect = ValueError(
            "Missing required field: newDatetime"
        )
    ...
    def test_inventory_addition_validation_error(self, mock_create_addition):
        ...
        mock_create_addition.side_effect = ValueError(
            "Missing required parameters: productTitle, inventoryToAdd"
        )
    ...
    def test_direct_event_body(self):
        ...
        event = {"actionType": "unsupported-action", "scheduleName": "test-schedule"}
