...
def create_scheduled_inventory_movements(event_body: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not all([schedule_name, group_name, new_datetime]):
        raise ValueError("❌ Missing required parameters! Must provide scheduleName, groupName, and newDatetime.")
    
    # Validate datetime format and convert timezone
    try:
        utc_dt = datetime.strptime(str(new_datetime), "%Y-%m-%dT%H:%M:%S").replace(tzinfo=ZoneInfo("UTC"))
    ...
    # Prepare the input for the MoveInventoryLambda
