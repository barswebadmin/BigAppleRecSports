...
def get_product_tags(product_id: str) -> list[str]:
    """Get current tags for a product."""
    product_gid = f"gid://shopify/Product/{product_id}"
    data = fetch_shopify(GET_PRODUCT_TAGS, {"id": product_gid})
    
    try:
        product = data.get('product') if data else None
        if not product:
            print(f"⚠️ Product {product_id} not found in Shopify response")
            return []
        
        tags = product.get('tags', [])
        
        # Handle both list and string formats from Shopify
        if isinstance(tags, list):
            # Already a list, return as-is (filter empty strings)
        ...
        print(f"❌ Error parsing product tags: {data}")
        raise ValueError(f"Could not get tags for product {product_id}") from e


def update_product_tags(product_id: str, tags: list[str]) -> Dict:
    """Update a product's tags. Replaces all existing tags with the provided list."""
    product_gid = f"gid://shopify/Product/{product_id}"
    
    # Convert list to comma-separated string for Shopify
    ...
    data = fetch_shopify(UPDATE_PRODUCT_TAGS, variables)
    
    user_errors = data.get("productUpdate", {}).get("userErrors", [])
    if user_errors:
        raise ValueError("Product tag update failed: " + '; '.join(
            f"{e['field']}: {e['message']}" for e in user_errors
        ))
