...
@click.option('--role', type=click.Choice(['MEMBER', 'OWNER', 'MANAGER'], case_sensitive=False), default='MEMBER', help='Member role (default: MEMBER)')
@click.pass_context
def add_member_cmd(
    ctx: click.Context,
    ...
    Examples:
        bars google groups add-member team-leads user
        bars google groups add-member team-leads user --role OWNER
        bars --json google groups add-member team-leads user
    """
    ...
    # API endpoint
    endpoint = f'http://localhost:8000/admin/google/groups/{group_email}/members'
        ...
        if response.status_code in (200, 201):
            ...
            if should_display:
                if json_output:
                    ...
                    _display_success_message(group_email, user_email, role, response_data)
...
def _display_success_message(
    ...
    """Display success message for member addition."""
    message = response_data.get('message', 'Member added successfully')
    
    # Check if this is a warning (member already exists)
    data = response_data.get('data', {})
    is_warning = data.get('warning', False)
    
    # Use warning icon for "already exists" case, success icon otherwise
    icon = "⚠️" if is_warning else "✅"
    
    click.echo(f"\n{icon} {message}", err=True)
    click.echo("=" * 60, err=True)
    click.echo(f"{'Group':<15} {group_email}", err=True)
    click.echo(f"{'Member':<15} {user_email}", err=True)
    click.echo(f"{'Role':<15} {role}", err=True)
    click.echo("=" * 60, err=True)
