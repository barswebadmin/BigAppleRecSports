...
class EstimateTierKind(StrEnum):
    ...
    @classmethod
    def from_raw(cls, raw: str) -> "EstimateTierKind":
        """Classify raw refund_to string to tier kind.

        Args:
            raw: Canonical refund_to value from the request.

        Returns:
            REFUND_TO_ORIGINAL for "original_method", STORE_CREDIT otherwise.
        """
        if (raw or "").lower() in ("original_method", "refund_to_original"):
            return cls.REFUND_TO_ORIGINAL
        return cls.STORE_CREDIT
